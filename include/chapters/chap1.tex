...
verifica facilmente che $\vec{u}^T \vec{A} \vec{x} \ge \vec{u}^T\vec{b}$.
Inoltre, è sempre possibile costruire $\vec{u}$ in modo che valga
$\vec{c}^T \ge \vec{u}^T \vec{A}$, da cui si trova
\begin{equation}\label{eq:weak_duality}
    \vec{c}^T \vec{x} \ge \vec{u}^T \vec{A} \vec{x} \ge \vec{u}^T\vec{b}
    \quad \forall \vec{x} \in F(\mathcal{P}),
\end{equation}
essendo $\vec{x} \ge 0$. Ed allora abbiamo trovato un limite inferiore alla
funzione obiettivo di $\mathcal{P}$, ossia $c_{0} = \vec{u}^T \vec{b}$. A
